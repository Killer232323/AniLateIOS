...
(async function(namespace) {
       ...
       // const fetcher = new window.Anilist.AnilistFetcher();
    ...
    const fetchFromUrl = async (url) => {
        try {
            ...
            return JSON.stringify(data.content.map(anime => {
                //console.log(`Anime title: ${anime.title_ru}, Status ID: ${anime.status.id}`);
                return {
                    url: anime.id,
                    title: anime.title_ru,
                    views: anime.views,
                    thumbnail_url: anime.image,
                    status: anime.status && anime.status.id ? status(anime.status.id) : 0
                };
            }), null, 2);
        } catch (error) {
            throw `Ошибка: ${error}`;
    ...
    const fetchFromDetails = async (id = 10907) => {
        try {
           ...
           // const url = `https://api.anixart.tv/related/${response.related.id}/0`;
            const related_data = await fetchFromUrl(`https://api.anixart.tv/related/${response.related.id}/0`);


           // const anilist = await fetcher.fetchMedia(response.title_original).catch(() => null);
            ...
            return JSON.stringify({
              ...
              //  background_url: anilist.bannerImage,
                status: status(response.status.id),
                viewing_order: related_data
            }, null, 2);
        } catch (error) {
            throw `Ошибка: ${error}`;
        }

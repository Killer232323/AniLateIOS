...
(async function(namespace) {
    ...
    const errorOptionsJson = JSON.stringify(errorOptions);

    const error = errorOptions.find(err => err.code === code);
    ...
    const fetchVideo = async (id) => {
        try {
            ...
            if (!data.types || data.types.length === 0) {
               await Promise.reject(new Error( JSON.stringify(error(2))));
            }
